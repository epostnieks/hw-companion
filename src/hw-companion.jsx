...
const Section = ({ number, title, subtitle }) => (
  <div style={{ marginTop: 48, marginBottom: 24 }}>
    <div style={{ fontFamily: FONTS.mono, fontSize: 12, color: ACCENT, letterSpacing: 2, marginBottom: 6 }}>{number}</div>
  ...
  return (
    <div style={{ background: BG, color: "rgba(255,255,255,0.8)", minHeight: "100vh", fontFamily: FONTS.serif }}>
      ...
      <header style={{ borderBottom: `1px solid ${BORDER}`, padding: "32px 0 24px" }}>
        <div style={{ maxWidth: 900, margin: "0 auto", padding: "0 24px" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
            <div>
              <div style={{ fontFamily: FONTS.mono, fontSize: 12, color: ACCENT, letterSpacing: 3, marginBottom: 12 }}>COMPANION DASHBOARD</div>
              ...
              <div style={{ fontFamily: FONTS.mono, fontSize: 12, color: MUTED, marginTop: 12 }}>Erik Postnieks · Working Paper v3.5 · March 2026</div>
            </div>
        ...
        <div style={{ maxWidth: 900, margin: "0 auto", padding: "0 24px", display: "flex", gap: 0 }}>
          {TABS.map(t => (
            <button key={t.id} onClick={() => setTab(t.id)} style={{
              fontFamily: FONTS.mono, fontSize: 13, padding: "12px 20px", border: "none", cursor: "pointer",
      ...
      <main style={{ maxWidth: 900, margin: "0 auto", padding: "0 24px 80px" }}>
        ...
        {tab === "cases" && (
          <div>
            ...
            <div style={{ fontFamily: FONTS.mono, fontSize: 13, color: ACCENT, marginBottom: 12, marginTop: 32 }}>HOLLOW WIN (0,1,1) — 6 CASES</div>
            {HOLLOW_WIN_CASES.map(c => (
              <Card key={c.name} color="rgba(220,38,38,0.15)">
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                  <div>
                    <span style={{ fontFamily: FONTS.mono, fontSize: 14, fontWeight: 600, color: "rgba(255,255,255,0.8)" }}>{c.name}</span>
                    <span style={{ fontFamily: FONTS.mono, fontSize: 12, color: MUTED, marginLeft: 8 }}>{c.years}</span>
                    <div style={{ fontFamily: FONTS.mono, fontSize: 12, color: MUTED, marginTop: 4 }}>{c.mechanism}</div>
                  </div>
                  <div style={{ textAlign: "right", whiteSpace: "nowrap" }}>
                    <div style={{ fontFamily: FONTS.mono, fontSize: 13, color: "#DC2626" }}>{c.cab}</div>
                    <div style={{ fontFamily: FONTS.mono, fontSize: 12, color: YELLOW }}>{c.penalties}</div>
                  </div>
                </div>
              </Card>
            ))}

            <div style={{ fontFamily: FONTS.mono, fontSize: 13, color: GREEN, marginBottom: 12, marginTop: 32 }}>WIN, WIN, WIN (1,1,1) — 5 CASES</div>
            {WIN_WIN_WIN_CASES.map(c => (
              <Card key={c.name} color="rgba(5,150,105,0.15)">
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                  ...
                  <div style={{ fontFamily: FONTS.mono, fontSize: 13, color: GREEN }}>{c.cab}</div>
                </div>
              </Card>
            ))}

            <div style={{ fontFamily: FONTS.mono, fontSize: 13, color: YELLOW, marginBottom: 12, marginTop: 32 }}>CONTESTED C — 7 CASES</div>
            {CONTESTED_CASES.map(c => (
              <Card key={c.name} color="rgba(245,158,11,0.15)">
                <span style={{ fontFamily: FONTS.mono, fontSize: 14, fontWeight: 600, color: "rgba(255,255,255,0.8)" }}>{c.name}</span>
                <div style={{ fontFamily: FONTS.mono, fontSize: 12, color: MUTED, marginTop: 4 }}>{c.c_options}</div>
              </Card>
            ))}
          </div>
        )}
        ...
        {tab === "calvano" && (
          <div>
            ...
            <Card highlight>
              <div style={{ fontFamily: FONTS.mono, fontSize: 12, color: MUTED, marginBottom: 8 }}>PARAMETERS</div>
            ...
            <div style={{ height: 300, marginTop: 24 }}>
              <ResponsiveContainer>
                <BarChart data={CALVANO_DATA} margin={{ left: 20, right: 20 }}>
                  ...
                  <XAxis dataKey="name" tick={{ fill: DIM, fontFamily: FONTS.mono, fontSize: 12 }} />
                  <YAxis tick={{ fill: MUTED, fontFamily: FONTS.mono, fontSize: 12 }} unit="%" />
                  <Tooltip contentStyle={{ background: "#1a1a2e", border: `1px solid ${BORDER}`, fontFamily: FONTS.mono, fontSize: 13 }} />
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 12, marginTop: 24 }}>
              <Card><div style={{ fontFamily: FONTS.mono, fontSize: 12, color: MUTED }}>HOLLOW WIN</div><div style={{ fontFamily: FONTS.mono, fontSize: 28, color: ACCENT }}>53%</div></Card>
              <Card><div style={{ fontFamily: FONTS.mono, fontSize: 12, color: MUTED }}>SYSTEM-DEGRADING</div><div style={{ fontFamily: FONTS.mono, fontSize: 28, color: "#DC2626" }}>77%</div></Card>
              <Card><div style={{ fontFamily: FONTS.mono, fontSize: 12, color: MUTED }}>SYSTEM-PRESERVING</div><div style={{ fontFamily: FONTS.mono, fontSize: 28, color: GREEN }}>23%</div></Card>
            </div>
            <Card highlight>
              <div style={{ fontFamily: FONTS.serif, fontSize: 15, color: DIM, lineHeight: 1.7, fontStyle: "italic" }}>
        ...
        {tab === "conflictoring" && (
          <div>
            ...
            {/* Phase label */}
            <div style={{ fontFamily: FONTS.mono, fontSize: 12, letterSpacing: 2, marginBottom: 16, color: cStep === 8 ? YELLOW : cStep < 5 ? ACCENT : GREEN }}>
              ...
              return (
                <div>
                  ...
                  <div style={{ display: "flex", alignItems: "baseline", gap: 12, marginBottom: 20 }}>
                    ...
                    <div>
                      <div style={{ fontFamily: FONTS.mono, fontSize: 16, fontWeight: 600, color: "rgba(255,255,255,0.9)" }}>{s.title}</div>
                      <div style={{ fontFamily: FONTS.mono, fontSize: 12, color: MUTED, marginTop: 2 }}>Step {s.step} of 8</div>
                    </div>
                  </div>
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
                    <div style={{ padding: "16px 20px", background: "rgba(255,255,255,0.02)", border: `1px solid ${BORDER}`, borderRadius: 2 }}>
                      <div style={{ fontFamily: FONTS.mono, fontSize: 11, color: MUTED, letterSpacing: 1, marginBottom: 8 }}>PROTOCOL</div>
                      <div style={{ fontFamily: FONTS.serif, fontSize: 16, color: "rgba(255,255,255,0.8)", lineHeight: 1.7 }}>
                        {s.desc}
                      </div>
                    </div>
                    <div style={{ padding: "16px 20px", background: "rgba(232,93,58,0.04)", border: `1px solid rgba(232,93,58,0.15)`, borderRadius: 2 }}>
                      <div style={{ fontFamily: FONTS.mono, fontSize: 11, color: ACCENT, letterSpacing: 1, marginBottom: 8 }}>LIBOR — WORKED EXAMPLE</div>
            ...
            {cStep === 8 && (
              <div>
                <div style={{ display: "flex", alignItems: "baseline", gap: 12, marginBottom: 20 }}>
                  ...
                  <div>
                    <div style={{ fontFamily: FONTS.mono, fontSize: 16, fontWeight: 600, color: "rgba(255,255,255,0.9)" }}>Whistleblower Infrastructure</div>
                    <div style={{ fontFamily: FONTS.mono, fontSize: 12, color: MUTED, marginTop: 2 }}>Tier 1 institutional exits — four federal programs</div>
                  </div>
                </div>

                <Card highlight>
                  <div style={{ fontFamily: FONTS.mono, fontSize: 12, color: YELLOW, letterSpacing: 1, marginBottom: 8 }}>CUMULATIVE ENFORCEMENT RECOVERIES</div>
                  <div className="pulse" style={{ fontFamily: FONTS.mono, fontSize: 36, color: YELLOW }}>$95B+</div>
                  <div style={{ fontFamily: FONTS.mono, fontSize: 12, color: MUTED, marginTop: 4 }}>Across SEC, CFTC, FCA, and IRS whistleblower programs</div>
                </Card>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginTop: 16 }}>
                  {WHISTLEBLOWER.map(w => (
                    <div key={w.program} style={{ padding: "16px 20px", background: "rgba(255,255,255,0.02)", border: `1px solid ${BORDER}`, borderRadius: 2 }}>
                      <div style={{ fontFamily: FONTS.mono, fontSize: 13, fontWeight: 600, color: "rgba(255,255,255,0.8)", marginBottom: 4 }}>{w.program}</div>
                      <div style={{ fontFamily: FONTS.mono, fontSize: 12, color: YELLOW, marginBottom: 8 }}>{w.statute}</div>
                      <div style={{ fontFamily: FONTS.mono, fontSize: 12, color: MUTED, lineHeight: 1.8 }}>
                        Reward: {w.reward}<br/>
                        Scope: {w.scope}<br/>
            ...
            <div style={{ display: "flex", justifyContent: "space-between", marginTop: 24 }}>
              <button onClick={() => setCStep(Math.max(0, cStep - 1))} disabled={cStep === 0} style={{
                fontFamily: FONTS.mono, fontSize: 13, padding: "8px 20px", border: `1px solid ${BORDER}`,
                background: cStep === 0 ? "transparent" : ACCENT + "15", color: cStep === 0 ? MUTED : ACCENT,
                borderRadius: 2, cursor: cStep === 0 ? "default" : "pointer",
              }}>← BACK</button>
              <div style={{ fontFamily: FONTS.mono, fontSize: 12, color: MUTED, alignSelf: "center" }}>
          ...
          <div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 12 }}>
              {[
                ...
                <Card key={d.domain}>
                  <div style={{ fontFamily: FONTS.mono, fontSize: 12, color: MUTED }}>{d.domain.toUpperCase()}</div>
                  <div style={{ fontFamily: FONTS.mono, fontSize: 24, color: d.color, marginTop: 4 }}>β = {d.beta}</div>
                  <div style={{ fontFamily: FONTS.mono, fontSize: 12, color: MUTED, marginTop: 8 }}>T* = {d.tstar}</div>
                  <div style={{ fontFamily: FONTS.mono, fontSize: 12, color: DIM }}>{d.class}</div>
                </Card>
              ))}
            </div>
            <Card highlight>
      ...
      <footer style={{ borderTop: `1px solid ${BORDER}`, padding: "20px 0", textAlign: "center" }}>
        <div style={{ fontFamily: FONTS.mono, fontSize: 12, color: "rgba(255,255,255,0.2)" }}>© 2026 Erik Postnieks · erik@woosterllc.com · All rights reserved</div>
      </footer>
    </div>
  );
}
